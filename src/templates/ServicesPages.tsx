...
export const config: TemplateConfig = {
  stream: {
    ...
    fields: ["name", "slug"],
    filter: {
      entityIds: ["services-global-data"],
    },
    localization: {
      locales: ["en_GB"],
      primary: false,
...
answersHeadlessConfig.verticalKey = "services";
const searcher = provideHeadless(answersHeadlessConfig);

const ServicesPages: Template<TemplateRenderProps> = ({ document }) => {
          ...
          <div className="header-bg">
            <div className="flex px-5 flex-col container-custom search_vertical">
              <div className="yext-search-bar container-custom-small">
                <YextSearchBar _site={_site} vertical={"services"} />
              </div>
              <Navigation />
            </div>
          </div>
